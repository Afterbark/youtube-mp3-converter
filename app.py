...
# Clients to try (fallbacks help dodge throttling/SABR)
CLIENTS_TO_TRY = [
    "web",
    "web_safari",
    "web_embedded",
    ...
    opts = {
        ...
        "extractor_args": {
            "youtube": {
                ...
                # intentionally NOT setting "player_skip" to allow webpage/config fallbacks
...
def download():
    ...
    try:
        ...
        # quick cleanup for direct downloads
        def _cleanup(path):
            try:
                time.sleep(30)
                Path(path).unlink(missing_ok=True)
